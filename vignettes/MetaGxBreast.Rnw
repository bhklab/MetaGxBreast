...
esets = MetaGxBreast::loadBreastEsets()[[1]]
@

This will load 39 expression datasets, with patients selected according to the default settings of the loadBreastEsets script. Users can modify the parameters of the function to restrict datasets that do not meet certain criteria from loading, some example parameters are shown below:
